...
// ------------------------- intent detect (algorithmic choose/recommend) -------------------------
function isAlgorithmIntent(query: string): boolean {
  const q = (query || '').toLowerCase()

  return (
    /(определи|подбери|что выбрать|порекомендуй|рекомендуй|какой тариф|какая услуга|какой пакет|какой продукт|подскажи что выбрать)/i.test(
      q
    )
  )
}
...
function buildSystemPrompt(query: string): string {
  ...
  const algoMode = isAlgorithmIntent(query)
...
Задача: работать в режиме алгоритмического подбора.
Если запрос содержит смысл "подбери/определи/что выбрать/порекомендуй/какой тариф/какая услуга" — действуй по логике "если → то", которая описана в контексте.

Правила:
- Проанализируй условия из запроса пользователя.
- Применяй только те правила/ветки "если → то", которые есть в контексте.
- Выдай ОДИН наиболее подходящий вариант (тариф/услуга/продукт).
- Коротко объясни почему именно он.
- Если не хватает данных для прохождения алгоритма — НЕ ДОДУМЫВАЙ. Задай ОДНО короткое уточнение, строго необходимое для выбора.

Формат (в своем ответе ты не пишешь "вопрос" или "ответ", а сразу отвечаешь):
[рекомендованный вариант + короткое обоснование]
...
Формат (в своем ответе ты не пишешь "вопрос" или "ответ", а сразу отвечаешь):
[краткий понятный ответ, как от менеджера]
